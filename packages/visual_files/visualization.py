...
def visualize(test_pred, train, p):
    ...
    df = pd.DataFrame(np.array(test_pred.cpu().detach().numpy()))
    df = df.set_index(pd.date_range(start='12/02/2017', end='12/31/2017'))
    df.columns = atm_machines

    tgcn_predict = pd.melt(df.reset_index(), id_vars='index')
...
def make_plot(tgcn_predict, train, p):
    ...
    for ax_,atm in zip(ax,tgcn_predict['atm'].unique()[:2]):
        ...
        print(f'wmape for {atm}: {np.abs(p_["y"] - tgcn_predict_["y_hat"]).sum() / np.abs(p_["y"]).sum():.4f}')
    fig.tight_layout()
    plt.show()