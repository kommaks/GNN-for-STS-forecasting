...
class GCN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        output = torch.tanh(ax @ self.weights)
        # (num_nodes, batch_size, output_dim)
        output = output.reshape((self._num_nodes, batch_size, self._output_dim))
        # (batch_size, num_nodes, output_dim)
        output = output.transpose(0, 1)

        outputs = torch.zeros(batch_size, self._input_dim, self._num_nodes).type_as(inputs)
